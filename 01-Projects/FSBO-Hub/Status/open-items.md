...
*Verified current 2026-09-22 — vault-sync: no code activity anywhere in the network since the 09-17 GSC sprint. A fresh **weekly network audit ran 2026-09-22** (superseding the 09-07 run cited below), still uncommitted in `buyer-hub/tools/network-audit/`. 7 "new" failures reported, none actually new: TN eForms PDF and `mohousing.com` (already tracked, unfixed), `dph.illinois.gov` + `idoi.illinois.gov` (now recurring a 4th+ time across two sites — increasingly looks genuinely broken rather than a crawler fluke, worth a manual re-check), `insurance.ohio.gov` (recurring geo-block, still un-allowlisted), plus 2 fsbo internal `??` transients (baltimore/catonsville, indianapolis/greenwood — expected noise). **The 09-07 run's 4× Redfin deep-link 404s and the `texas.gov` root 404 no longer appear as failures** — self-resolved or were transient, drop from active triage. Mirror-branch drift unchanged: still the same 22 repos. The auction-hub stale-deploy question (main vs 7-commits-ahead master) is unchanged/unverified — not re-confirmed this run. See [[network-status]].*
*Previously: 2026-09-18 — vault-sync: a 4-commit GSC-Coverage-driven SEO sprint landed on both fsbo product repos, 2026-09-14→17 (see [[network-status]], [[SESSION-2026-09-17]]). Closes the long-carried [[fsbo-suburb-thin-content]] item below (pruning applied on top of the 07-19 enrichment fix) and fixes a previously-unknown sitemap bug (1,281 URLs were all redirects, not canonical). No other repo moved. The still-open 09-07 audit findings below (auction-hub stale deploy, TN eForms PDF, 4 Redfin deep links, MO housing link, insurance.ohio.gov geo-block, texas.gov root, mirror-branch drift) remain unactioned and unchanged — no new weekly audit ran this window.*
*Previously: 2026-09-14 — vault-sync re-run: still no repo activity network-wide since 2026-09-05 (full 44-directory scan repeated, empty). Nothing new to triage beyond the still-open 09-07 audit findings below — those items (auction-hub stale deploy, TN eForms PDF, 4 Redfin deep links, MO housing link, insurance.ohio.gov geo-block, texas.gov root, mirror-branch drift) remain unactioned and unchanged.*
*Previously: 2026-09-11 — vault-sync re-run: still no repo activity network-wide since 2026-09-05 (full scan repeated, empty). Nothing new to triage beyond the still-open 09-07 audit findings below.*
*Previously: 2026-09-10 — vault-sync: **no repo activity network-wide since 2026-09-05** (`git log --all --since` empty across every repo). Nothing shipped, nothing new committed or pushed. **Correction:** the 🔴 "plaintext GitHub PAT in `fsbo-freemium-sandbox/deploy_freemium_branch.ps1`" item (below, 2026-07-23) is **resolved on disk** — that file was rewritten 2026-08-26 to remove the hardcoded PAT and read `$env:GH_TOKEN` from an encrypted SecretManagement vault. It was part of a 2026-08-25/26 credential overhaul (9 tokens flagged "burned", encrypted `ops` vault, global secret-blocking pre-commit hook, ops scripts moved out of the Obsidian vault) — see updated item and [[network-status]]. Provider-side rotation of the 9 tokens still shows pending in the ops-scripts README, not verifiable from here. Ties to [[vault-credential-exposure]]. The 09-07 weekly-audit findings below are unchanged and still awaiting triage/approval.*
...
## 🔴 Open — network audit (2026-09-22 run, supersedes 09-07), see [[link-audit]]

- **Real external link rot, unchanged, still not applied**:
...
- **Illinois pair now recurring a 4th+ time — likely genuinely broken, not a crawler fluke.** `dph.illinois.gov/topics-services/birth-death-other-records.html` (funeral-hub) and `idoi.illinois.gov/consumers/consumerinsurance/homeownerrenter.html` (insurance-hub) were dismissed as transient in 07-27 and 08-05/06, then flagged again 08-18 and now 09-22. Worth an actual manual browser check next interactive session rather than assuming it self-clears again.
- **`insurance.ohio.gov` geo-block** — recurring again, still unactioned: add to `allowlist.json` `geo_blocked_404_hosts` + `node audit.mjs --update-baseline`. Fine for US visitors.
- **Resolved/dropped from active triage (09-22 re-run no longer shows these):** the 4× Redfin "recently sold" deep-link 404s (`/albany/`, `/anchorage/`, `/atlanta/`, `/bakersfield/`) and the `texas.gov`/`www.texas.gov` root 404 (land-hub TX page) — both gone from this run's failure list, likely were transient/geo-flaky rather than real. Also the `trec.texas.gov` PDF that was previously broken is now in the "resolved since baseline" list.
- **auction-hub stale deploy question — not re-verified this run.** As of 09-07: `auction.byownerhub.com` live served `origin/main` while `origin/master` was 7 commits ahead with unshipped P3b/P8 SEO content (clean FF). The 09-22 report still lists auction-hub in mirror-branch drift (`main=7b4b533 master=c5df642`) with 0 link failures on the live site — consistent with the deploy still being stale, but not independently re-confirmed. Fix: FF `main` → `origin/master`, or CF Pages retry if it deploys from `master`; also confirm auction-hub's intended prod branch (`new-build-hub` has the opposite drift yet already serves `master`, so the mapping isn't consistent network-wide).
- **Mirror-branch drift: 22 repos, unchanged, 4th weekly run standing** (same repo list as 09-07: frbo, buyer, commercial, firsttimebuyer, flatfee, biz, estate, funeral, mobile, lien, llc, timeshare, inspection, trust, rv, eviction, moto, str, mortgage, new-build, foreclosure, auction). All clean FFs. Needs a directional per-repo batch resync — no single push fixes all 22.
- 2 fsbo internal `??` transients this run (`baltimore/catonsville`, `indianapolis/greenwood`) — expected crawler noise, no action.
- Full report: `buyer-hub/tools/network-audit/report.md` + `results.json` (modified-but-uncommitted in the working tree, now holding the 2026-09-22 run; not yet triaged/applied — action next interactive session or via the Slack-approval execute routine).

## ✅ Resolved 2026-09-17 — GSC Coverage sprint: sitemap redirect bug + suburb-page pruning (fsbo product repos, pushed)

See [[SESSION-2026-09-17]], [[network-status]]. Triggered by the user reading a 2026-09-14 Search Console Coverage export directly (not the weekly network audit, which is external-link-focused and doesn't touch GSC data). Production (`fsbo-freemium-sandbox`) `master` `4bf79a7` → `600a4b6`, 4 commits, all pushed; `fsbo-staging` independently re-built the same set (`cb86a99` → `b8c161d`).

- **Sitemap was serving 1,281 redirects, not canonical URLs** (`67ca152`/`49376d2`) — trailing-slash mismatch with this app's `trailingSlash: false` (308s `/x/` → `/x`). Likely explains why Google had stopped re-reading the sitemap after 7 July (69 days stale). Fixed: trailing slashes dropped, `lastmod` added to ~1,050 URLs that never had one, split into a `/sitemap.xml` index over six section files. **Do not copy this sitemap-normalization logic to the CF Pages sibling sites** — they redirect the opposite direction, so their identical-looking logic was already correct.
- **Closes [[fsbo-suburb-thin-content]]** — 535 of 751 suburb pages (zero GSC impressions/clicks in 91 days) are now `noindex,follow` and out of the sitemap (`503ab6c`/`608d4c5`); pages stay live/linked, just stop competing for a crawl allowance Google was visibly rationing (43 metro pages crawled-and-declined, 25 state guides never crawled at all in the same export). The 216-slug allowlist (`SUBURBS_WITH_DEMAND` in `src/lib/suburb-indexing.ts`) is meant to be refreshed from a fresh GSC export in about a month — worth checking it actually got refreshed next time this file is touched.
- Also landed same sprint, not bugs: a homepage positioning/copy change (`9ff1a08`/`c9b96ca`, "unbundling" framing) and a new public `/commission-index` page (`600a4b6`/`b8c161d`) built as press/backlink bait — pitch doc exists (`docs/commission-index-pitches.md`) but not confirmed sent to anyone yet.

- [ ] **Re-check GSC Coverage in ~4-6 weeks** to see whether the sitemap fix + suburb pruning actually moved the crawled/indexed numbers — the fix is evidence-based triage, explicitly not guaranteed to work (see the code comment in `suburb-indexing.ts`).
- [ ] **Confirm whether the commission-index pitch doc was ever sent** to press contacts, and if so, follow up on results.
