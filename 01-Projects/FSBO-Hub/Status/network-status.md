...
- **CORRECTION — the untracked deploy scripts no longer hold a plaintext credential.** `deploy_freemium_branch.ps1` was rewritten 2026-08-26 (mtime confirms): the hardcoded `ghp_v078…` PAT is removed; it now `. "…\ops-scripts\_secrets.ps1"` and uses `$env:GH_TOKEN`, resolved at runtime from an encrypted SecretManagement vault (`ops`), process-only. `deploy.bat` never contained a token (plain `git add . && commit && push` on ambient creds). The vault carried "plaintext PAT, file mtime unchanged since 07-23, not rotated" across ~6 weeks of sessions — that mtime claim was wrong from 2026-08-26 on.
- **2026-08-25/26 credential-hygiene overhaul (newly logged).** Per `C:\Users\kevc_\Documents\ops-scripts\README.md`: 9 credentials found exposed in never-version-controlled local scripts + one git commit (4 Cloudflare tokens → consolidated onto `CF_API_TOKEN`, DNS one kept as `CF_API_TOKEN_2`; 4 GitHub credentials — 2 classic PATs + 2 OAuth tokens → all map to `GH_TOKEN`; 1 Netlify PAT → `NETLIFY_TOKEN`). All flagged "burned — rotate all nine at the provider." Every ops script rewritten to resolve tokens from the encrypted `ops` vault (nothing written to disk/registry). ops scripts moved out of the Obsidian vault to `…\ops-scripts` (vault `.gitignore` now blocks `*.ps1`/`*.bat`/`*.zip`). Global pre-commit hook at `~/.githooks/pre-commit` (`git config --global core.hooksPath`) now blocks any commit containing a CF/GitHub/Netlify/AWS/Stripe/Slack token or private key, in every repo. **Still open:** provider-side rotation of the 9 tokens — the README still says "rotate all nine", not verifiable from this machine. Ties to [[vault-credential-exposure]].
- **Unchanged:** `freemium-wip` 11 commits unpushed (~5 weeks / ten vault checks, deploy-readiness unverified since 07-12, likely superseded by production's real Stripe build); mirror-branch drift ~22 repos (3rd weekly run standing, all clean FFs, mostly cosmetic — see [[open-items]]); local-clone pointer staleness (origin has everything): funeral-hub `master` -7, investor-hub `master` -6, landlord-hub `claude/fervent-almeida-217d54` -20; fsbo-hub's ~8 stale `claude/*` worktree branches behind `origin/main`.

---

## 🟢 Previous State of Union (2026-09-04 evening)
