...
*Previously: 2026-08-24 — weekly link audit ([[link-audit-latest]]): 21 new failures, 11 real (orphan sitemap gaps on buyer/divorce/estate/funeral-hub), 3 real external 404s (2 with replacements found), 7 false positives, 155 resolved. Mirror-branch drift jumped 0→19 repos, worth a look. See 🔴 section below.*
...
## 🔴 Open — network audit (2026-08-05/06 run), see [[link-audit-latest]]
...
- **Slack posting skipped** (connector unauthenticated in this session, and Kevin turned off all Slack integrations 2026-07-26 — see [[bugs-feature-routines]]). All proposals are in [[link-audit-latest]] for direct action instead.
...
## 🔴 Open — network audit (2026-07-27 run), see [[link-audit-latest]]
...
## 🔴 NEW — plaintext GitHub PAT in untracked file, new sandbox repo (2026-07-23)

A new repo `kevc55-code/fsbo-freemium-sandbox` appeared on disk (not previously documented here), seeded from fsbo-hub's `freemium-wip` as an isolated Stripe test-mode sandbox. Its **untracked** `deploy_freemium_branch.ps1` contains a **live-looking GitHub PAT in plaintext** (`ghp_v078...`). It was correctly excluded from the git commit (message says so explicitly) so it isn't pushed, but it still sits in plaintext on local disk in a repo with a configured remote. **Recommend Kevin revoke/rotate this token** (GitHub → Settings → Developer settings → Personal access tokens) regardless of push status. **Still not rotated as of 2026-08-21 — file mtime unchanged since 07-23 (29 days).** See [[SESSION-2026-07-23]] §4.
...
## 🔴 Open — network audit (2026-07-23 run), see [[link-audit-latest]]
